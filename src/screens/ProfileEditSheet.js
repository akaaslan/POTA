...
function ChipGroup({ options, selected, onSelect, compact }) {
  return (
    <View style={[pe.chips, compact && pe.chipsCompact]}>
      {options.map(function(opt) {
        var active = selected === opt;
        return (
          <TouchableOpacity
            key={opt}
            style={[pe.chip, compact && pe.chipCompact, active && pe.chipActive]}
            onPress={function() { onSelect(opt); }}
            activeOpacity={0.75}
          >
            {active ? <View style={pe.chipGlow} /> : null}
            <Text style={[pe.chipTxt, compact && pe.chipTxtCompact, active && pe.chipTxtActive]}>{opt}</Text>
          </TouchableOpacity>
        );
      })}
    </View>
          ...
          <View style={pe.sheet}>
            ...
            >
              ...
              <View style={pe.section}>
                <SectionHead num={1} title={t('profileEdit.section_identity')} />
                <View style={pe.identityCard}>
                  <View style={pe.nicknameWrap}>
                    <Text style={pe.inputLabel}>{t('profileEdit.field_nickname')}</Text>
                    <TextInput
                      style={pe.input}
                      value={form.nickname || ''}
                      onChangeText={function(v) { set('nickname', v); }}
                  ...
                  <View style={pe.jerseyWrap}>
                    <Text style={pe.inputLabel}>{t('profileEdit.field_jersey')}</Text>
                    <View style={pe.jerseyBox}>
                      <Text style={pe.jerseyHash}>#</Text>
                      <TextInput
                        style={pe.jerseyInput}
                        value={form.jerseyNumber || ''}
              ...
              <View style={pe.section}>
                <SectionHead num={2} title={t('profileEdit.field_position')} />
                <ChipGroup options={POSITIONS} selected={form.position} onSelect={function(v) { set('position', v); }} />
              </View>

              {/* 03 — OYUNCU TİPİ */}
              <View style={pe.section}>
                <SectionHead num={3} title={t('profileEdit.field_archetype')} />
                <ChipGroup options={ARCHETYPES} selected={form.archetype} onSelect={function(v) { set('archetype', v); }} />
              </View>

              {/* 04 — SEVİYE */}
              <View style={pe.section}>
                <SectionHead num={4} title={t('profileEdit.field_level')} />
                <ChipGroup options={EXPERIENCES} selected={form.experience} onSelect={function(v) { set('experience', v); }} />
              </View>

              {/* 05 — SEMT */}
              <View style={pe.section}>
                <SectionHead num={5} title={t('profileEdit.field_district')} />
                <ChipGroup options={DISTRICTS} selected={form.district} onSelect={function(v) { set('district', v); }} compact />
              </View>
            ...
            <View style={pe.footer}>
              <TouchableOpacity
                style={[pe.saveBtn, saving && pe.saveBtnDim]}
                onPress={handleSave}
                disabled={saving}
                activeOpacity={0.85}
              >
                {saving ? (
                  ...
                  <View style={pe.saveBtnInner}>
                    <Text style={pe.saveBtnTxt}>{t('profileEdit.save_btn')}</Text>
                    <Text style={pe.saveBtnArrow}>→</Text>
                  </View>
                )}
              </TouchableOpacity>
            </View>
...
const pe = StyleSheet.create({
  ...
  handle: { width: 40, height: 4, borderRadius: 2, backgroundColor: C.border, alignSelf: 'center', marginTop: S.sm, marginBottom: S.xs },
  ...
  headerCenter: { flex: 1, alignItems: 'center', overflow: 'hidden' },
  headerGlow: { position: 'absolute', width: 140, height: 44, borderRadius: 70, backgroundColor: C.orange, opacity: 0.07, top: -14 },
  headerTitle: { color: C.text, fontSize: F.base, fontWeight: '900', fontStyle: 'italic', letterSpacing: 1.5, textTransform: 'uppercase' },
  closeBtn: { width: 32, height: 32, borderRadius: R.full, backgroundColor: C.bgCard2, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: C.border },
  closeIcon: { color: C.textDim, fontSize: 13, fontWeight: '700' },

  scroll: { paddingHorizontal: S.screen, paddingTop: S.lg },
  section: { marginBottom: S.x2 },

  // Section head
  sectionHead: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: S.base },
  sectionNum: { color: C.lime, fontSize: 11, fontWeight: '900', letterSpacing: 1, minWidth: 22 },
  ...
  nicknameWrap: { flex: 1 },
  jerseyWrap: { width: 80, alignItems: 'center' },
  inputLabel: { color: C.textMuted, fontSize: 9, fontWeight: '900', letterSpacing: 2, marginBottom: S.sm, textTransform: 'uppercase' },
  ...
  textarea: { height: 80, paddingTop: 12, fontSize: F.sm },
  charCount: { color: C.textMuted, fontSize: 9, fontWeight: '700', letterSpacing: 1, textAlign: 'right', marginTop: S.xs },
  ...
  jerseyHash: { color: C.textMuted, fontSize: F.sm, fontWeight: '900', letterSpacing: 1 },
  jerseyInput: { color: C.lime, fontSize: F.x2, fontWeight: '900', letterSpacing: -1, textAlign: 'center', padding: 0, width: '100%' },

  // Chips
  chips: { flexDirection: 'row', flexWrap: 'wrap', gap: S.sm },
  chipsCompact: { gap: S.xs },
  chip: {
    paddingHorizontal: S.base,
    paddingVertical: 10,
    borderRadius: R.full,
  ...
  chipCompact: { paddingHorizontal: S.md, paddingVertical: 7 },
  chipActive: { borderColor: C.lime, backgroundColor: 'rgba(200,240,0,0.08)' },
  chipGlow: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(200,240,0,0.06)' },
  chipTxt: { color: C.textDim, fontSize: F.xs, fontWeight: '700', letterSpacing: 0.5 },
  chipTxtCompact: { fontSize: 10 },
  chipTxtActive: { color: C.lime, fontWeight: '800' },
  ...
  saveBtnInner: { flexDirection: 'row', alignItems: 'center', gap: S.sm },
  saveBtnTxt: { color: '#fff', fontSize: F.sm, fontWeight: '900', letterSpacing: 2.5, textTransform: 'uppercase' },
  saveBtnArrow: { color: '#fff', fontSize: F.md, fontWeight: '900' },
});
